...
while(exit == 0):											# Run LoggerShell until the exit flag is set
	...
	if(userInput == "get-battery-voltage"):									# User Option: Read the battery voltage
		...
		command[1] = "python LoggerReportSwap.py 255 255 255 255 255 255 255 255 255 255 1 255"			# Command to start a logging session
		...
		command[0] = "python LoggerReportSwap.py 255 255 255 255 255 255 255 255 255 255 255 255"		# Command to read report data
		command[1] = "python LoggerReportSwap.py 255 255 255 255 255 255 255 255 255 255 0 255"			# Command to stop a logging session
		command[2] = "python LoggerReadRom.py"									# Command to read the EEPROM chip (to obtain remaining data in logging session)
		process = subprocess.Popen(command[0], stdout=subprocess.PIPE, shell=True)				# Create a sub-process to run command[0]
		(output, err) = process.communicate()									# Assign variables in which to store output
		p_status = process.wait()										# Run the sub-process
		outputList = output.split()										# Extract the output
		...
		p_status = process.wait()										# Run the sub-process
		outputList = output.split()										# Extract the output
		Date = outputList[1].zfill(2) + "/" + outputList[2].zfill(2) + "/" + "20" + outputList[0].zfill(2)	# Format date information
		Time = outputList[3].zfill(2) + ":" + outputList[4].zfill(2) + ":" + outputList[5].zfill(2)		# Format time information
		...
		newMonth = raw_input("> Month (mm): ")									# Prompt user to input the current month
		newDay = raw_input("> Day (dd): ")									# Prompt user to input the current day
		newYear = raw_input("> Year (yy): ")									# Prompt user to input the current year
		newHour = raw_input("> Hour (hh): ")									# Prompt user to input the current hour
		newMinute = raw_input("> Minute (mm): ")								# Prompt user to input the current minute
		newSecond = raw_input("> Second (ss): ")								# Prompt user to input the current second
		space = " "
															# Command to set a new date and time
		command[0] = "python LoggerReportSwap.py " + newYear + space + newMonth + space + newDay + space + newHour + space + newMinute + space + newSecond + " 255 255 255 255 255 255"
		command[1] = "python LoggerReportSwap.py 255 255 255 255 255 255 255 255 255 255 255 255"		# Command to read report data
		process = subprocess.Popen(command[0], stdout=subprocess.PIPE, shell=True)				# Create a sub-process to run command[0]
		(output, err) = process.communicate()									# Assign variables in which to store output
		p_status = process.wait()										# Run the sub-process
		process = subprocess.Popen(command[1], stdout=subprocess.PIPE, shell=True)				# Create a sub-process to run command[1]
		...
		p_status = process.wait()										# Run the sub-process
		outputList = output.split()										# Extract the output
		Date = outputList[1].zfill(2) + "/" + outputList[2].zfill(2) + "/" + "20" + outputList[0].zfill(2)	# Display date
		Time = outputList[3].zfill(2) + ":" + outputList[4].zfill(2) + ":" + outputList[5].zfill(2)		# Display time
		print ">", Date
		print ">", Time
		...
		newPeriod = raw_input("> New Period (integer value between 1 and 254) or C to CANCEL: ")		# Prompt the user for a new period (or cancel)
		if(newPeriod != "C"):											# If the user decided not to cancel
			command[0] = "python LoggerReportSwap.py 255 255 255 255 255 255 255 255 255 255 255 " + newPeriod	# Command to set the new period
			command[1] = "python LoggerReportSwap.py 255 255 255 255 255 255 255 255 255 255 255 255"		# Command to read report data
			process = subprocess.Popen(command[0], stdout=subprocess.PIPE, shell=True)				# Create a sub-process to run command[0]
			(output, err) = process.communicate()									# Assign variables in which to store output
			p_status = process.wait()										# Run the sub-process
			...
			print "> Period Set:", newPeriod									# Display the newly set period to the user
		else:													# Else
		...
		newID = raw_input("> New ID: ")										# Prompt the user for a new ID number
		...
		newSite = raw_input("> New Site: ")									# Prompt the user for a new site number
		...
		newMeterRez = raw_input("> New Meter Resolution: ")							# Prompt the user for a new meter resolution
